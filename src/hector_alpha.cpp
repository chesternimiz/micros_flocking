...
double pm1=0.3,pm2=1.5,pm3=0.3;
...
int main(int argc, char** argv)
{
   ...
   while(ros::ok())
   {
     ...
     cout<<f_g().first<<' '<<f_d().first<<' '<<temp.first<<"_______";
      cout<<f_g().second<<' '<<f_d().second<<' '<<temp.second<<endl;
     /*
      ...
      if(my_height<4)
          sendmsg.linear.z = 0.1;
      else if (my_height<=4.2)
          sendmsg.linear.z = 0;
      else
          sendmsg.linear.z = -0.1;
      if(!diffdrive)
      ...
      lastmsg.linear.x = sendmsg.linear.x; //for !diff_drive, in robot frame rather than map frame
