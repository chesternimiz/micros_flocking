...
class MotivationalBehavior(FunctionUnit):
    active_behavior = [0 for row in range(100)]
   ...
   # def start_motive(self):
    ...
    def calculate_activity_suppression(self):
         ...
         if self._debug_mode :
             active_set = -1
             for i in range(0,10):
                if MotivationalBehavior.active_behavior[i] == 1:
                    active_set = i
             if(active_set==4):
                active_set = 3
             self._f = file(self._filename,'a+')
             self._f.write(str(self._time))
             self._f.write(' ')
             self._f.write(str(active_set))
             self._f.write('\n')
             self._f.close()
...
if __name__ == '__main__':
    try:
         mb = MotivationalBehavior('bs1','switch0/activate')
         mb.run()
